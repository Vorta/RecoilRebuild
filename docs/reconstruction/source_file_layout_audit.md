...
| `[0x401710,0x401b20)` | `ai_net.h: steering_substate_leaf_layer` | proven own-header contributor | Steering substate dispatcher plus basic move/turn, offset-target, and dynamic-offset steering leaves. BN assembly/xrefs prove `0x401945..0x401948` is alignment, `0x401948..0x401964` is the seven-entry dispatcher table, and `0x401964..0x401970` is twelve unreferenced `0x90` padding bytes before the real `0x401970` body; the former BN tailcall-forwarder label was an analysis artifact, not a function row. |
...
`0x4016a0` has no retail lexical-scope discriminator. Current assembly shows a
four-argument `__fastcall` helper used only by the two direct calls in
`0x401580`, with no `this`, AINet instance access, table/address-taking xref,
or other-TU caller. VC5 scratch objects prove both the current static-member
form and a file-static/free form can emit the body, direct `REL32` calls, and
the retail caller/helper order; only the discarded decorated symbol and COFF
storage class distinguish them. The free form also happens to make a
reconstruction-only inline-assembly call operand legal, but that convenience
is not source ownership evidence. Keep the static-member/file-static question
explicitly unresolved until original `.obj`, PDB/CodeView, MAP/ILK/browser
records, exact-revision source, or another lexical/linkage discriminator is
recovered. ChatGPT Pro independently BLOCKed changing lexical ownership:
static-member and file-static forms remain observationally tied in the stripped
binary, and exact body/caller bytes prove neither lexical scope nor access
control. Keep the current static-member spelling provisional pending one of
those independent discriminators.
...
cleanup forwarding thunk to the authored `HudUiFillBitmap` destructor core.
...
byte coverage before it can support owner byte readiness, but that dependency
...
EH frame. The hard-byte review prioritized real C++ destructor/EH structure
for `0x403ed0`, separate boolean/register lifetimes for `0x404070`, and
source-level lifetime/allocator probes for `0x403930`; it rejected raw assembly
because EH metadata and allocator choices must arise naturally.
...
byte-match review ranked ordinary C++ branch and lifetime changes first,
preserved the sample/handle and variant branch semantics, and rejected separate
manual destruction plus `operator delete` for the shutdown path; the retained
zero-byte forms satisfy those constraints.
...
changes. The hard-byte review identified a real C++ destructor as the
source-faithful route to VC5 EH/unwind/member-teardown emission and rejected
synthetic try/catch, manual member teardown, and raw assembly.
...
Pro ranked declaration- and lifetime-only register-coloring probes first and
treated raw assembly as premature; every useful probe retained the 153-mismatch
baseline. A focused
...
`mov edx,1; sete cl; sub edx,ecx`. ChatGPT Pro ranked source-level
boolean/register-lifetime probes and judged raw inline assembly premature; none
improved on the 23-mismatch best result, and no raw assembly or allowlist row
was added.
...
prove new emitted `.h` rows. A 2026-07-09 ChatGPT Pro critique independently
ALLOWed only this order-diagnostic status and frontier advance under the stated
limitations.
...
Three 2026-07-09 ChatGPT Pro critiques were used narrowly: one ranked the
command-binding fallback while refusing raw assembly, one required body-aware
cleanup-label remapping, and one ALLOWed the HUD frontier/docs update only as
