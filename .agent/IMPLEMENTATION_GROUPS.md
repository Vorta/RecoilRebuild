...
### Group: zNetwork session-runtime local-player owner/data cleanup

- Anchor: 0x48a9c0 zNetwork_DPlay::CreateLocalPlayerRecordAndRegister, with
  lower owner/data blockers 0x48a310, 0x48b660, 0x48ae70, 0x48afe0,
  0x489f80, 0x48b940, and dependency 0x48c200.
- Queue: active source-owner/data cleanup for the G016 zNetwork DirectPlay
  local-player, player-list, receive-buffer, host/color, and packet-dispatch
  runtime cluster.
- Reason: HudUiNetGameSetupPanel_LaunchButton now routes past zOpt and
  session-create blockers; the next visible dependency blocker is this shared
  zNetwork session-runtime owner/data cluster.
- Current evidence:
  - Source-owner mapper classifies the slice as authored source-file/subsystem
    ownership in `src/GameZRecoil/zNetwork/znet_dplay.cpp` plus declarations
    and records in `zNetwork.h`, not a C++ class, vtable, or provider table.
  - DirectPlay `EnumPlayers`, `CreatePlayer`, `GetPlayerCaps`, and `Receive`
    calls are DirectX provider ABI dependencies. The packet handler list is an
    authored data-driven callback list, not a C++ virtual table.
  - Data classifier reports no anchor in this slice is eligible for
    `Data ❎`; each touches authored globals or authored runtime records.
  - Existing `znetwork_directplay_runtime_globals` VC5 data-symbol evidence
    covers only part of this cluster. The highest-priority mismatch is
    `g_zNetworkCurrentPlayerCountCached`: BN data evidence says the initial
    dword is one, while current source initializes the global to `0`.
  - Functional target `znetwork_alloc_free_player_color_index` now has a
    registered native smoke in `tests/native/smoke.cpp` and linked smoke
    implementation in `tests/native/znetwork_identity_smokes.cpp`; the target
    passes after the x86 native smoke rebuild.
- Current blockers:
  - Fix the `g_zNetworkCurrentPlayerCountCached` initializer mismatch, then
    collect or extend ignored VC5 data-symbol evidence for the touched
    zNetwork runtime globals before any `Data ✅` or tier B promotion in this
    slice.
  - Do not promote `0x489f80` or `0x48b940` as standalone leaves; keep owner
    acceptance tied to the coherent zNetwork session-runtime source owner.
- Next action:
  - Assign a narrow source-worker pass for the cached-player-count initializer
    mismatch in `znet_dplay.cpp`, then rerun focused zNetwork functional
    targets and reassess data-symbol coverage.
