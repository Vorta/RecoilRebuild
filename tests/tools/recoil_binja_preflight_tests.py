...
from _recoil.commands.binja_preflight import validate_binaries, validate_probe, validate_status  # noqa: E402
...
class RecoilBinjaPreflightTests(unittest.TestCase):
    ...
    def test_validate_binaries_rejects_inactive_expected_database(self) -> None:
        ...
        self.assertIn("open but not active", errors[0])
        self.assertIn("Activate", errors[1])

    def test_validate_binaries_accepts_inactive_database_when_active_not_required(self) -> None:
        binaries = {
            "binaries": [
                {"filename": EXPECTED_FILE, "active": False},
            ]
        ...
        self.assertEqual([], validate_binaries(binaries, expected_file=EXPECTED_FILE, require_active=False))
