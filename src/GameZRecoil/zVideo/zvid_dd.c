...
namespace zVideo_dd
{
    namespace
    {
        ...
        /**
         * Original-source helper evidence: source-faithful helper recovered from address-backed callers in this source
         * file. Purpose: provide the recovered PageUnlockBeforeRelease helper behavior for zVideo callers.
         */
...
namespace zVideo_dd
{
     ...
     */
    int __fastcall OpenVideoMode(int)
    {
...
namespace zVideo_dd
{
    ...
    BltSwToPrimaryRect(zVidImagePartial * srcImage, int srcColorKeyEnable, zVidRect32* srcRect, zVidRect32* dstRect)
    {
        ...
        const DWORD bltFlags = DDBLT_WAIT + DDBLT_ASYNC + (srcColorKeyEnable != 0 ? DDBLT_KEYSRC : 0);

        int clipped = zVideo_buff::ClipCoordToRange(&dstRectLocal.left, 0, g_zVideo_PrimarySurfaceState.width - 1);
        if (clipped < 0) {
        ...
        clipped = zVideo_buff::ClipCoordToRange(&dstRectLocal.right, 0, g_zVideo_PrimarySurfaceState.width);
        if (clipped < 0) {
            return;
        }
        if (clipped > 0) {
        ...
        clipped = zVideo_buff::ClipCoordToRange(&dstRectLocal.top, 0, g_zVideo_PrimarySurfaceState.height - 1);
        if (clipped < 0) {
        ...
        clipped = zVideo_buff::ClipCoordToRange(&dstRectLocal.bottom, 0, g_zVideo_PrimarySurfaceState.height);
        if (clipped < 0) {
            return;
        }
        if (clipped > 0) {
...
namespace zVideo_dd
{
    ...
    {
        if (g_zVideo_UseHalfResBackbuffer == 0 && g_zVideo_pSelectedHwApiDeviceRecord->m_deviceFeatureFlags == 0) {
            return 0;
        }

        const unsigned int caps
            = (g_zVideo_pSelectedHwApiDeviceRecord->m_deviceFeatureFlags != 0 ? DDSCAPS_NONLOCALVIDMEM : 0)
            + DDSCAPS_VIDEOMEMORY;
...
namespace zVideo_dd
{
    ...
    int __cdecl CreateHalfResBackbufferSurfaces()
    {
        ...
        HRESULT hresult
            = CreateSurface3FromDesc(g_zVideo_pDirectDraw2, &desc, &g_zVideo_DisplayModeSurfaceState.surf, 0);
        if (hresult != DD_OK) {
        ...
        hresult = CreateSurface3FromDesc(g_zVideo_pDirectDraw2, &desc, &g_zVideo_SwSurfaceState.surf, 0);
        if (hresult != DD_OK) {
...
namespace zVideo_dd
{
    ...
    int __cdecl CreateFullscreenSoftwareSurfaces()
    {
        ...
        hresult = CreateSurface3FromDesc(g_zVideo_pDirectDraw2, &desc, &g_zVideo_PrimarySurfaceState.surf, 0);
        if (hresult != DD_OK) {
...
namespace zVideo_dd
{
    ...
    int __fastcall InitFullscreenSoftwarePixelPack(IDirectDrawSurface3 * displaySurface)
    {
        ...
        if (pixelFormat.dwGBitMask == 0x03e0) {
            zVideo::PixelPackSetupFromMasks(5, 5, 5, pixelFormat.dwRBitMask, 0x03e0, pixelFormat.dwBBitMask);
            return 0;
        }
        ...
        TeardownVideoSubsystem();
        zError::ReportOld(0x800, g_zVideo_SourceFile_ZvidDdC, 0x5bd, g_zVideo_UnrecognizedPixelFormatMsg);
        return 0x5a56ffff;
    }
...
namespace zVideo_dd
{
    ...
    BOOL CALLBACK EnumDirectDrawDeviceCallback(GUID * guid, LPSTR driverDescription, LPSTR driverName, LPVOID context)
    {
        ...
        zVidHwApiDeviceRecordPartial* entry = &g_zVideo_HwApiDeviceTable[g_zVideo_NumAcceptedDirectDrawDevices];
...
namespace zVideo_dd
{
    ...
    )
    {
        ...
        if (entry->m_acceptedD3DDeviceCount >= 4) {
            TeardownVideoSubsystem();
            zError::ReportOld(0x800, g_zVideo_SourceFile_ZvidDdC, 0x7d3, g_zVideo_D3DEnumTooManyDriversMsg);
            return 0;
        }
