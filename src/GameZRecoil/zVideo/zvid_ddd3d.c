...
#define AppendFanCloseVertexIfNeeded(vertices, count)                                                                  \
    ...
    int __cdecl BeginSceneAndFlushPendingRenderStates()
    {
        ...
        if (g_zVideo_PendingDitherEnable != -1) {
            g_zVideo_pD3DDevice->SetRenderState(D3DRENDERSTATE_DITHERENABLE, (DWORD)(g_zVideo_PendingDitherEnable));
        ...
        for (;;) {
            if (blitPrimaryToSwFirst != 0) {
                ...
                g_zVideo_SwSurfaceState.surf
                    ->Blt((RECT*)(dstRect), g_zVideo_PrimarySurfaceState.surf, (RECT*)(srcRect), bltFlags, 0);
    ...
    int __cdecl CreateDeviceState()
    {
        ...
        HRESULT hresult
            = g_zVideo_pDirectDraw2->CreateSurface(&zBufferDesc, (IDirectDrawSurface**)(&g_zVideo_pZBufferSurface), 0);
        if (hresult != DD_OK) {
        ...
        hresult
            = g_zVideo_SwSurfaceState.surf->AddAttachedSurface((IDirectDrawSurface3*)(g_zVideo_pZBufferAttachSurface));
        if (hresult != DD_OK) {
        ...
        if (hresult == DD_OK && image->palette != 0) {
            ...
            hresult
                = g_zVideo_pDirectDraw2
                      ->CreatePalette(DDPCAPS_8BIT | DDPCAPS_ALLOW256, (LPPALETTEENTRY)(image->palette), &ddPalette, 0);
            if (hresult == DD_OK) {
    ...
    )
    {
        ...
        if (zVideo_dd::LockSurfaceWaitRestore((IDirectDrawSurface3*)(textureRecord->m_uploadSurface), &lockedDescOut)
            == 0) {
    ...
    {
        return zVideo_dd::UnlockSurfaceWaitRestore((IDirectDrawSurface3*)(textureRecord->m_uploadSurface)) == 0 ? 1 : 0;
    }
    ...
    void __fastcall DrawPointColor16(zVideo_XyzVertex * pointPos, unsigned int packedColor16, int pointCount)
    {
        ...
        const HRESULT hresult
            = g_zVideo_pD3DDevice
                  ->DrawPrimitive((D3DPRIMITIVETYPE)(1), (D3DVERTEXTYPE)(3), g_zVideo_D3DSubmitTempVertices, 1, 0);
        if (hresult != DD_OK) {
    ...
    void __cdecl FlushQuadBatch()
    {
        ...
        for (unsigned int i = 0; i < (unsigned int)(g_zVideo_QuadBatchCount); ++i) {
            g_zVideo_pD3DDevice
                ->DrawPrimitive(D3DPT_TRIANGLEFAN, (D3DVERTEXTYPE)(3), g_zVideo_QuadBatchItemsBase[i].vertices, 4, 0);
        }
