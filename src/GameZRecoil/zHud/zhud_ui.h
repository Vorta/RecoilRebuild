...
struct HudUiElement {
    ...
    HudUiElement* Constructor(int x, int y);
    ...
    virtual void SetBltSourceAndClipRect(void* bltSourceOrNull, const HudUiRect* rectOrNull);
    ...
    virtual void OnPointerButtonState(int x, int y);
    virtual void OnCapturedPrimaryRelease();
    virtual int ShouldHandleInput(HudUiBackground* background, int hovered);
    virtual void AfterInputUpdate(HudUiBackground* background, int hovered);
    virtual int HitTest(int px, int py);
    ...
    unsigned char HitTestTrue(int px, int py);
};
...
struct HudUiWidget : HudUiElement {
    ...
    zVidImagePartial* SetImageBorrowedAndInvalidate(zVidImagePartial* image);
...
struct HudUiContainer {
    ...
    int FindChildWithPrev(HudUiElement* child, HudUiElement** previousOut);
...
void __fastcall UpdateMarkersAndProgressFromVariantTag(const zTag4Partial* requiredVariantTag);
...
void __fastcall SetModeCounterState(int counterIndex, int state);
...
int __fastcall InitHudLayouts(const HudUiRect* displaySection, const HudUiRect* windowSection);
...
struct StdPtrVector {
    ...
    void ClearNoOpDestroy(int* begin, int* end);
};
...
struct HudUiPrimitiveBindTarget : HudUiElement {
    ...
    void SetSegmentEndpoints(int startX, int startY, int endX, int endY);
};
...
struct HudUiTextLabel : HudUiElement {
     ...
     */
    HudUiTextLabel() { }
    HudUiTextLabel(const char* text, int x, int y, int flags);
    HudUiTextLabel(const HudUiTextLabel& source);
    HudUiTextLabel* ConstructorWithPosAndFlags(const char* text, int x, int y, int flags);
...
struct HudUiCircle : HudUiElement {
     ...
     * identity.
     */
    HudUiCircle(int x, int y, int circleRadius, unsigned int circleColor565);
    virtual void Draw();
    int HitTest(int px, int py);
    unsigned char HitTestCore(int px, int py);
};
...
struct HudUiBackgroundCursorWidget : HudUiWidget {
    ...
    HudUiBackgroundCursorWidget(const char* imagePath, int captureEnabled);
    ...
    void RebuildCapturedImage(int x, int y);
    void Draw();
    void DrawBase();
};
...
struct HudUiBackgroundMemberCursorWidget : HudUiBackgroundCursorWidget {
     ...
     */
    HudUiBackgroundMemberCursorWidget(const char* imagePath, int captureEnabled)
        : HudUiBackgroundCursorWidget(imagePath, captureEnabled)
    {
    }
};
...
struct HudUiZrdWidget : HudUiWidget {
    ...
    virtual int LoadFromZrd(zReader::Node* zrdSection, HudUiBackground* ownerDialog);
...
struct HudUiCheckToggleWidget : HudUiZrdWidget {
    ...
    int LoadFromZrd(zReader::Node* zrdSection, HudUiBackground* ownerDialog);
...
struct HudUiCycleSelectorWidget : HudUiZrdWidget {
    ...
    void Update(float deltaSeconds);
    void AddTextEntry(int index, const char* text, int posX, int posY);
    void ApplyFontStyleForEntry(int index, int styleIndex);
    void AddBitmapEntry(int index, const char* imagePath, int posX, int posY);
    int LoadFromZrd(zReader::Node* zrdSection, HudUiBackground* ownerDialog);
};
...
struct HudUiPanel : HudUiTextLabel {
    ...
    virtual void
    SetFont(const char* faceName, int height, int weight, int width, int italic, int charSet, int pitchAndFamily);
    virtual void SetFontHandle(HGDIOBJ fontHandle);
    virtual void __cdecl SetTextFmtV(const char* format, va_list args);
     ...
     */
    void SetTextColorsAndMarkDirty(unsigned int color0, unsigned int color1)
    {
        textColor0 = color0;
        textColor1 = color1;
        textDirty = 1;
     ...
     */
    unsigned int SetShadow(unsigned int enableShadow, int offsetX, int offsetY)
    {
    ...
    int MeasureTextPrefixRect(int maxChars, RECT* outRect);
    int QueryTextWidth()
    {
        if (textDirty != 0) {
            RebuildTextRect();
...
struct HudCmdBindingVector {
    ...
    /**
     * Original-source helper; no standalone retail function exists.
     * Evidence: recovered in the HUD source cluster near address-backed 0x4b92a0
     * HudUiListSelectorItem::HudUiListSelectorItem callers. Purpose: preserve the recovered HUD behavior for
     * HudCmdBindingVector.
     */
    HudCmdBindingVector()
    {
     ...
     */
    HudCmdBindingEntry* const* begin() const
    {
        return first;
    }
    /**
     ...
     */
    HudCmdBindingEntry* const* end() const
    {
        return last;
    }
    unsigned int size() const;
    HudCmdBindingEntry** erase(HudCmdBindingEntry** eraseFirst, HudCmdBindingEntry** eraseLast);
...
struct HudCmdBindingEntry : HudCmdBinding {
     ...
     * HudCmdBindingEntry.
     */
    HudCmdBindingEntry(const char* text, int id)
    {
 ...
 * Evidence: recovered in the HUD source cluster near address-backed 0x4b92a0
 * HudUiListSelectorItem::HudUiListSelectorItem callers. Purpose: preserve the recovered HUD behavior for
 * HudCmdBindingVector::PushBack.
 */
inline void HudCmdBindingVector::push_back(HudCmdBindingEntry* entry)
{
    ...
    if ((unsigned int)(limit - insertPos) < 1u) {
        ...
        HudCmdBindingEntry** const newBegin
            = (HudCmdBindingEntry**)(::operator new((unsigned int)newCapacityCount * sizeof(HudCmdBindingEntry*)));
...
struct HudCmdBindButtonBase : HudUiCheckToggleWidget {
    ...
    int AddBindingEntry(const char* displayText, int commandId);
    ...
    int LoadFromZrd(zReader::Node* zrdSection, HudUiBackground* ownerDialog);
    void RebuildBindingSlotWidgets(int totalCount, int visibleCount);
};
...
struct HudUiMessage : HudUiWidget {
    ...
    int LoadWeaponLayoutFromNode(zReader::Node* layoutNode, const HudUiPanelFontParams* fontParams);
    static void __fastcall SelectVariantDisplay(int messageIndex, int variantIndex);
    static void __fastcall ApplySideImageSwap(int messageIndex, int sideIndex);
    static void __fastcall ClearDisplay(int messageIndex);
    static void __fastcall SetValueIfOwnerMatches(int messageIndex, int ownerSideIndex, float valueOrClearToken);
    static void __fastcall
    UpdateSelectedWeaponDisplay(int weaponBankIndex, int weaponSideIndex, float valueOrClearToken);
};
...
struct HudUiTextInput {
    ...
    int ShiftTextRight(int count, int startPos);
    int ShiftTextLeft(int count, int startPos);
};
...
struct HudUiOwnedTextInput : HudUiTextInput {
     ...
     * HudUiOwnedTextInput.
     */
    HudUiOwnedTextInput(int bufferSize)
        : HudUiTextInput(bufferSize)
        , owner(0)
    {
    }
    virtual void OnAccept();
...
struct HudUiNumericTextInput : HudUiZrdWidget {
    ...
    virtual void OnAccept() { }
    void OnActivate();
    static int __fastcall RawKeyboardCallback(int key, HudUiNumericTextInput* callbackCtx);
};
...
RECOIL_STATIC_ASSERT(offsetof(HudUiNetGameSetupOverlayOwner, m_dialog) == 0x04);
...
struct HudUiTransitionTextPanel : HudUiPanel {
    ...
    void SetFlashColorAndRate(unsigned int flashColor, float flashRate);
};
...
#define g_HudUiMgrActiveWeaponMessageIndex (g_HudUiMgr.activeMessageSelection.activeWeaponMessageIndex)
#define g_HudUiMgrActiveWeaponSideIndex (g_HudUiMgr.activeMessageSelection.activeWeaponSideIndex)
...
#define g_HudUiLoadingCheckpointRawProgress (g_HudUiMgr.loadingCheckpointTable.checkpointProgressRaw)
#define g_HudUiLoadingCheckpointProgress (g_HudUiMgr.loadingCheckpointTable.checkpointProgressNormalized)
#define g_HudUiLoadingCheckpointProgressScale (0.0186219737f)
#define g_HudUiLoadingCheckpointMaxIndex (g_HudUiMgr.loadingCheckpointTable.maxCheckpointIndex)
#define g_HudUiLoadingCheckpointCurrentIndex (g_HudUiMgr.loadingCheckpointTable.currentCheckpointIndex)
#define g_HudUiLoadingCheckpointCurrentProgress (g_HudUiMgr.loadingCheckpointTable.currentProgress)
#define g_HudUiMgrTailBar (g_HudUiMgr.tailBar)

struct HudUiFlashPanel {
    static unsigned int __fastcall ComputeFlashBlendColor(unsigned int color0, unsigned int color1, float blend);
};
...
struct HudUiCompositePanel : HudUiPanel {
     ...
     */
    void LayoutEntries(int x, int y)
    {
        SetPos(x, y);
    }
    void ResizeEntryVectorAndRelayout(int entryCount);
    void ReapplyEntryCount();
    void ResizeEntryCount(int oldCount, int entryCount);
    void __cdecl SetTextFmt(const char* format, ...);
    void __cdecl SetTextFmtV(const char* format, va_list args);
    virtual void ScrollHistory();
    void SetFont(const char* faceName, int height, int weight, int width, int italic, int charSet, int pitchAndFamily);
    void Update(float deltaSeconds);
...
struct HudUiBackground : HudUiBackgroundContainer {
    ...
    zReader::Node* LoadFromZrd(const char* zrdPath, const char* sectionName, int capturePrimary);
    zReader::Node* LoadZrdAndSection(zReader::Node* loadedRootNode, const char* sectionName, int capturePrimary);
    virtual void SetEnabled(int enabled);
    unsigned char __fastcall
    BindButtonsNodeToWidgetByName(zReader::Node* parentNode, HudUiWidget* widget, const char* name);
    int BindWidgetByName(zReader::Node* loadedSectionNode, HudUiWidget* widget, const char* name);
    int BindPrimitiveNodeToElement(zReader::Node* loadedSectionNode, HudUiElement* element, const char* name);
...
struct HudCmdDialog : HudUiBackground {
    ...
    int ApplyPrimaryKeyRebind(int keyCode, int commandIndex);
    int ApplySecondaryKeyRebind(int keyCode, int commandIndex);
    int ApplyJoystickButtonRebind(int buttonCode, int commandIndex);
    int ApplyMouseButtonRebind(int buttonCode, int commandIndex);
};
...
struct HudUiMessageBoxDialog : HudUiBackground {
    ...
    HudUiMessageBoxDialog(const char* zrdPath, const char* sectionName);
    ~HudUiMessageBoxDialog();
    int RunModal(const char* messageText, const char* titleText, void* modalContext = 0, float timeoutSeconds = -1.0f);
    virtual void OnOk();
    virtual void OnCancel();
};
...
struct HudUiPanelLayoutEntry {
     ...
     */
    HudUiPanelLayoutEntry(const char* text, int x, int y)
        : panel(text, x, y)
    {
    }
    ...
    static void __stdcall DestroyRange(HudUiPanelLayoutEntry* start, HudUiPanelLayoutEntry* end);
#endif
};
...
struct HudUiPanelSpan {
    ...
    void insert(iterator insertPos, unsigned int count, const HudUiPanelLayoutEntry& templatePanel)
    {
        InsertN(insertPos, count, &templatePanel);
    }
    void insert(iterator insertPos, const HudUiPanelLayoutEntry& templatePanel)
    {
    ...
    void InsertN(HudUiPanelLayoutEntry* insertPos, unsigned int count, const HudUiPanelLayoutEntry* templatePanel);
    void DestroyAndFree();
};
...
struct HudUiPanelSpanVec {
    ...
    void insert(iterator insertPos, unsigned int count, const HudUiPanelSpan& templateSpan)
    {
        InsertN(insertPos, count, &templateSpan);
    }
    void insert(iterator insertPos, const HudUiPanelSpan& templateSpan)
    {
        InsertN(insertPos, 1, &templateSpan);
    }
    void InsertN(HudUiPanelSpan* insertPos, unsigned int count, const HudUiPanelSpan* templateSpan);
};
...
struct HudUiPanelSimple : HudUiPanel {
    inline HudUiPanelSimple(const char* text = 0, int x = 0, int y = 0);
};
...
struct HudUiTimerPanel : HudUiPanelSimple {
    ...
    static void __stdcall SetSeconds(float elapsedSeconds, float secondsStep);
    static float GetSeconds();
    static void __fastcall ZarWriteTimerDataCallback(zZbdSectionCallbackCtx* sectionCtx, HudUiTimerPanel* userData);
    static void __stdcall ZarReadTimerData(const float* buffer, int byteCount, HudUiTimerPanel* userData);
    void Update(float deltaSeconds);
...
struct HudUiTextStack4 : HudUiContainer {
    ...
    HudUiPanel* PushLine(const char* message, float duration);
    void Clear();
    void SetFontAll(const char* faceName, int height, int weight, int width);
    void SetTextColors(unsigned int color0, unsigned int color1);
    void SetXAll(int x);
...
RECOIL_STATIC_ASSERT(offsetof(HudLayoutHW, widget0) == 0x30);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, sensorRangeSq) == 0x50);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, trackedProgressSlot) == 0x54);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, sensorPanel) == 0x58);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, sensorOverlay) == 0x114);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, sensorMeter) == 0x1d0);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, shieldMessageWidget) == 0x310);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, stringMenu) == 0x314);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, targetMarkerImages) == 0x318);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrSensorBlock, targetMarkerCount) == 0x32c);
...
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, selected) == 0x14c);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, ownerSelector) == 0x150);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, itemIndex) == 0x154);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, mouseRectValid) == 0x158);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, mouseRect) == 0x15c);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, selectedImage) == 0x16c);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, unselectedImage) == 0x170);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, selectedRolloverImage) == 0x174);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioButtonWidget, unselectedRolloverImage) == 0x178);
RECOIL_STATIC_ASSERT(sizeof(CHudRadioGroupWidget) == 0x17c);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioGroupWidget, optionCount) == 0x14c);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioGroupWidget, options) == 0x150);
RECOIL_STATIC_ASSERT(offsetof(CHudRadioGroupWidget, selectedIndex) == 0x178);
...
RECOIL_STATIC_ASSERT(offsetof(HudCmdBindingVector, first) == 0x04);
RECOIL_STATIC_ASSERT(offsetof(HudCmdBindingVector, last) == 0x08);
RECOIL_STATIC_ASSERT(offsetof(HudCmdBindingVector, limit) == 0x0c);
#endif
...
RECOIL_STATIC_ASSERT(offsetof(HudCmdDialog, captureState) == 0xcdfc);
...
#if !defined(_MSC_VER) || _MSC_VER >= 1200
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpan, first) == 0x04);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpan, last) == 0x08);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpan, limit) == 0x0c);
#endif
RECOIL_STATIC_ASSERT(sizeof(HudUiPanelSpanVec) == 0x10);
#if !defined(_MSC_VER) || _MSC_VER >= 1200
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpanVec, first) == 0x04);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpanVec, last) == 0x08);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanelSpanVec, limit) == 0x0c);
#endif
...
RECOIL_STATIC_ASSERT(offsetof(HudUiMessage, layoutX) == 0x388);
RECOIL_STATIC_ASSERT(offsetof(HudUiMessage, layoutY) == 0x38c);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiNumericTextInput, textInput) + offsetof(HudUiOwnedTextInput, owner) == 0x25c);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, hudRootPanel) == 0x1c);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, reticleWidget) == 0x364);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, nanitePanel) == 0x420);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, objective) == 0x690);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, sensor) == 0xbcc);
RECOIL_STATIC_ASSERT(sizeof(HudUiMgrSensorBlock) == 0x330);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, weaponSlots) == 0xefc);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, reticleSnapRadiusSq) == 0x46fc);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, weaponState) == 0x4700);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, unknown_4704) == 0x4704);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, timerPanel) == 0x4784);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, timerPanelFloat) == 0x4788);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, activeModeCounterIndex) == 0x478c);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, modeCounters) == 0x4790);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, activeMessageSelection) == 0x4b10);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, messages) == 0x4b18);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, statsList) == 0x7610);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, statsListState5) == 0x7624);
RECOIL_STATIC_ASSERT(offsetof(HudLoadingCheckpointTable, checkpointProgressRaw) == 0x4);
RECOIL_STATIC_ASSERT(offsetof(HudLoadingCheckpointTable, checkpointProgressNormalized) == 0x68);
RECOIL_STATIC_ASSERT(offsetof(HudLoadingCheckpointTable, maxCheckpointIndex) == 0xcc);
RECOIL_STATIC_ASSERT(offsetof(HudLoadingCheckpointTable, currentCheckpointIndex) == 0xd0);
RECOIL_STATIC_ASSERT(offsetof(HudLoadingCheckpointTable, currentProgress) == 0xd8);
RECOIL_STATIC_ASSERT(sizeof(HudLoadingCheckpointTable) == 0xdc);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, loadingCheckpointTable) == 0x7628);
RECOIL_STATIC_ASSERT(offsetof(HudUiMgrData, tailBar) == 0x7704);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, textColor0) == 0x14c);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, textColor1) == 0x150);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, hFont) == 0x154);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, cachedText) == 0x15c);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, textWidthPx) == 0x25c);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, textHeightPx) == 0x260);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, shadowEnabled) == 0x264);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, bkMode) == 0x268);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, textRect) == 0x28c);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, shadowOffsetX) == 0x29c);
RECOIL_STATIC_ASSERT(offsetof(HudUiPanel, shadowOffsetY) == 0x2a0);
RECOIL_STATIC_ASSERT(sizeof(HudUiPanelSimple) == 0x2a4);
RECOIL_STATIC_ASSERT(sizeof(HudUiTimerPanelFloat) == 0x2b0);
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanelFloat, sampleElapsedSec) == 0x2a4);
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanelFloat, displayValue) == 0x2a8);
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanelFloat, sampleFrameCount) == 0x2ac);
...
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanel, elapsedSeconds) == 0x2a4);
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanel, stopped) == 0x2a8);
RECOIL_STATIC_ASSERT(offsetof(HudUiTimerPanel, secondsStep) == 0x2ac);
