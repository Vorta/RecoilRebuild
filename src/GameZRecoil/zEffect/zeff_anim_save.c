...
namespace
{
    ...
    const unsigned int kMaxActivationSaveRecordSize = offsetof(zEffectAnimActivationSaveRecord, trackedNodes)
...
namespace zEffect_Anim
{
     ...
     */
    void __fastcall LoadActivationRecords(void*, const char* sectionToken, void* data, int, void*)
    {
        ...
        if (savedState == 4) {
            if (entry->activationState != 4) {
                ...
                zError::ReportOld(0x100, g_zEffect_SourceFile_ZeffAnimSaveC, 0x1f5, g_zEffectAnim_StateInvalidMsg);
            }
            if (record->base.nodeToken == -1) {
                memcpy(AllocActivationRecord(), &record->base, sizeof(zEffectAnimActivationRecord));
            }
    ...
    LoadRunningAnimRecords(void* unused, const char* sectionToken, void* data, int dataSize, void* extraCtx)
    {
        ...
        FILE* const tempStream = zUtil_ZBD::OpenTempReadStream(data, dataSize);
        if (tempStream == 0) {
            return;
        }
        ...
        CZClass::gwNodeSetActionCallbackTail(entry->runtimeNode, (void*)(&zEffect_Anim::RunSequence));
    ...
    void __fastcall LoadAnimRecords(void* unused, const char* sectionToken, void* data, int dataSize, void* extraCtx)
    {
        ...
        for (int i = 0; i < header->trackedNodeCount; ++i) {
            ...
            if (record->usesCachedMatrix != 0) {
                ...
                CZObject3D::gwObject3DSetScale(node, record->transform[6], record->transform[7], record->transform[8]);
            }
        }
    }
...
namespace zEffect
{
    ...
    int __fastcall ActivateRuntimeEntryAtPosition(zEffect_RuntimeEntry * runtimeEntry, const zVec3* worldPos)
    {
        ...
        CZObject3D::gwObject3DSetScale(runtimeEntry->effectNode, currentScale, currentScale, currentScale);
        CZObject3D::gwObject3DSetPosition(runtimeEntry->effectNode, worldPos->x, worldPos->y, worldPos->z);
