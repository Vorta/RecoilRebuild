...
struct zIndexArchive {
    ...
    void* OpenFileByName(const char* filename, unsigned int* outSize);
    int ReadFileByName(const char* filename, void* buffer, unsigned int* bufferSize);
};
...
typedef int(__fastcall* zArchiveListCompare)(void*, void*);

zArchiveList* __cdecl zArchiveListNew();
int __fastcall zArchiveListFree(zArchiveList* list);
void __fastcall zArchiveListLink(zArchiveListNode* after, zArchiveListNode* newNode, zArchiveListNode* before);
int __fastcall zArchiveListAddHead(zArchiveList* list, void* payload);
...
zArchiveListNode* __fastcall zArchiveListFindNode(zArchiveList* list, void* payload);
...
void* __fastcall zArchiveListFindCompare(zArchiveList* list, zArchiveListCompare predicate, void* userData);
void* __fastcall zArchiveListFindKey(zArchiveList* list, unsigned int value);
void* __fastcall zArchiveListFind(zArchiveList* list, zArchiveListCompare predicate, void* userData);
int __fastcall zRdrStrCmpPredicate(void* str1, void* str2);
int __fastcall zRdrSearchPathContainsFilePredicate(void* searchDir, void* filename);
char* __fastcall zRdrResolvePathInSearchPathList(zArchiveList* searchPathList, const char* filename);
FILE* __fastcall zRdrOpenFileResolved(zArchiveList* searchPathList, const char* filename, const char* mode);
...
zReader::Node* __fastcall zRdrFindNode(zReader::Node* node, const char* searchName, int startIndex);
zReader::Node* __fastcall zRdrGetNode(zReader::Node* parentNode, const char* name);
zReader::Node* __fastcall zRdrAllocNode(int headerWord, int fieldCount);
int __fastcall zReaderReadString(void* hFile, zReader::Value* outString);
int __fastcall zRdrRead(void* hFile, zReader::Node* outNode);
...
void* __fastcall zArchiveListGet(zArchiveList* list, int index);
}
