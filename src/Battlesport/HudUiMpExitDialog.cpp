...
namespace {
template <typename Method> unsigned int MpExitMethodAddress(Method method)
{
...
HudUiWidget_FTable MakeMpExitDialogButtonFTable(unsigned int activateCallback)
{
...
const HudUiWidget_FTable g_HudUiZrdWidget_MpExitDialog_NewGameButton_Vtbl =
    MakeMpExitDialogButtonFTable(
        MpExitMethodAddress(&HudUiMpExitDialog_NewGameButton::OnActivate));
...
// (D:\Proj\Battlesport\HudUiMpExitDialog.cpp)
RECOIL_NOINLINE void RECOIL_THISCALL HudUiMpExitDialog::Update(float deltaSeconds)
{
    if (m_mpNewGameButtonMode >= 0)
    {
        ...
        HudScoreboard::SetScaleAndRebuild(fadeElapsedSeconds < 1.0f
                                              ? fadeElapsedSeconds
                                              : 1.0f);
    }
...
RECOIL_NOINLINE void RECOIL_THISCALL HudUiMpExitDialog::LoadLayout()
{
    ...
    zVideo::Fx_SetSurfaceState(image->pixels, imageWidth, image->height,
                               imageWidth * 2);
    zVideo::buff_BlurRegionByMode(0, 3);
    zVideo::buff_BlurRegionByMode(0, 3);
    zVideo::buff_BlurRegionByMode(0, 3);

    HudScoreboard::SetScaleAndRebuild(0.0f);

    zReader::Node *const loadedSection = base.LoadFromZrd("dialog.zrd", "MPEXIT", 1);
    if (loadedSection != 0)
    {
        if (m_mpNewGameButtonMode >= 0)
        {
            base.BindWidgetByName(loadedSection, &m_mpNewGameButton.base, "MPNEWGAME");
        }
        base.BindWidgetByName(loadedSection, &m_mpExitButton.base, "MPEXITBTN");
...
// (D:\Proj\Battlesport\HudUiMpExitDialog.cpp)
HudUiMpExitDialog *RECOIL_THISCALL
...
RECOIL_NOINLINE void RECOIL_THISCALL RecoilApp_MpExitDialogState::OnEnter()
{
    if (g_HudUiMpExitDialog == 0)
    {
        HudUiMpExitDialog *dialog =
            (HudUiMpExitDialog *)::operator new(sizeof(HudUiMpExitDialog));
        if (dialog != 0)
        {
            ...
            dialog->m_mpExitButton.base.ftable =
                &g_HudUiZrdWidget_MpExitDialog_ExitButton_Vtbl;
            dialog->base.base.base.vptr =
                (const HudUiContainer_FTable *)&g_HudUiMpExitDialog_Vtbl;
        }
...
RECOIL_NOINLINE int RECOIL_THISCALL RecoilApp_MpExitDialogState::OnUpdateShouldQuit()
{
    ...
    HudUiMpExitDialog *const dialog = g_HudUiMpExitDialog;
    typedef void (RECOIL_THISCALL *UpdateFn)(HudUiMpExitDialog *self,
                                             float deltaSeconds);
    const HudUiMpExitDialog_Vtbl *const vtable =
        (const HudUiMpExitDialog_Vtbl *)dialog->base.base.base.vptr;
    ((UpdateFn)vtable->slots[0])(dialog, g_FrameDeltaTimeSec);
