...
DEFAULT_VC6_ROOT = DEFAULT_VC6_ROOT_BASE
    ...
    *,
    ...
    for item in functions_data:
        ...
        if not isinstance(symbol, str):
            raise ValueError(f"{path}: expected function symbol as a string for {address}")
        symbol_regex = item.get("symbol_regex")
        if symbol_regex is not None and (not isinstance(symbol_regex, str) or not symbol_regex):
            raise ValueError(f"{path}: expected non-empty symbol_regex string for {address}")
        if not symbol and symbol_regex is None:
            raise ValueError(f"{path}: expected function symbol or symbol_regex for {address}")
        if symbol_regex is not None:
            try:
                re.compile(symbol_regex)
            except re.error as exc:
                raise ValueError(f"{path}: invalid symbol_regex for {address}: {exc}") from exc
...
def compare_compiled_selections(
    *,
    ...
    if any(function.symbol_regex for selection in selections for function in selection.functions):
            ...
            try:
                ...
                if function.symbol_regex is not None:
                    if target.compare_mode != "coff_bytes":
                        raise ValueError(
                            "symbol_regex is currently supported only for coff_bytes targets"
                        )
                    assert coff_object is not None
                    pattern = re.compile(function.symbol_regex)
                    matches = [
                        symbol.name
                    ...
                    if len(matches) != 1:
                        detail = "no matches" if not matches else "matches: " + ", ".join(matches[:8])
                        if len(matches) > 8:
                            detail += f", ... ({len(matches)} total)"
                        raise ValueError(
                            f"symbol_regex for {function.address} did not resolve uniquely: {detail}"
                        )
                    resolved_function = replace(function, symbol=matches[0])
