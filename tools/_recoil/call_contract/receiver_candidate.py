...
def _exact_candidate_cfg_vptr_proofs(
    ...
    *,
    ...
    No nullable spelling is erased. Both ECX and the loaded vptr must derive
    from the same registered call result or exact array receiver after
    convergence, including loops. Relocated receiver arithmetic is excluded.
    ...
    for instruction_index, (instruction, address) in enumerate(zip(instructions, addresses)):
        if _cc_cfg._instruction_mnemonic(instruction) not in {
            ...
            continue
        destination = _cc_cfg._instruction_operand(instruction).split(",", 1)[0].strip()
        if destination not in _cc_catalog.REGISTER_STATE_NAMES:
            continue
        offset = address - caller_start
    ...
    for index, instruction in enumerate(instructions):
        ...
        exact_array = ("affine(" in value
            and "call-result(" not in value
            and _eligible_retail_fresh_targetless_vptr(value))
        if value == f"load({receiver})" and (receiver in known_results or exact_array):
            result[index] = value
    return result
...
def _exact_cfg_zero_stride_seeds(
    ...
    """Prove zero-entry nonvolatile cursors with one exact update per cycle.
