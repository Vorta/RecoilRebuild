...
from _recoil.lib.pe import PeFormatError, PeSection, data_directory, hex32, parse_pe_headers, rva_to_offset
from _recoil.lib.tooling import write_if_changed, write_text_if_changed
    ...
    for parts, data_rva, data_offset, size, codepage in walk_resource_directory(
        ...
        entries.append(
            ResourceEntry(
                ...
                size=size,
                codepage=codepage,
                payload_path=raw_resource_filename_for_identity(type_id, name, language),
            )
        )
...
def raw_resource_filename_for_identity(
    type_id: int | str, name: int | str, language: int
) -> str:
    """Return the stable, human-readable payload path for a typed identity."""

    type_part = resource_type_label(type_id)
    name_part = resource_file_token(name)
    return f"{type_part}_{name_part}_{language:04x}.bin"


def parse_message_table(payload: bytes) -> list[MessageTableEntry]:
    if len(payload) < 4:
        raise ResourceExtractError("message table resource is too small")
    block_count = struct.unpack_from("<I", payload, 0)[0]
    table_end = 4 + block_count * 12
    if table_end > len(payload):
        raise ResourceExtractError("message table block headers exceed payload size")

    entries: list[MessageTableEntry] = []
    for block_index in range(block_count):
        low_id, high_id, offset = struct.unpack_from("<III", payload, 4 + block_index * 12)
        if high_id < low_id:
            raise ResourceExtractError(f"message table block {block_index} has descending id range")
        cursor = offset
        for message_id in range(low_id, high_id + 1):
            if cursor + 4 > len(payload):
                raise ResourceExtractError(f"message table entry {hex32(message_id)} is truncated")
            length, flags = struct.unpack_from("<HH", payload, cursor)
            if length < 4 or cursor + length > len(payload):
                raise ResourceExtractError(f"message table entry {hex32(message_id)} has invalid length")
            raw_text = payload[cursor + 4 : cursor + length]
            if flags & 0x1:
                if raw_text.endswith(b"\0\0"):
                    raw_text = raw_text[:-2]
                text = raw_text.decode("utf-16le", errors="replace")
            else:
                if raw_text.endswith(b"\0"):
                    raw_text = raw_text[:-1]
                text = raw_text.decode("latin-1", errors="replace")
            entries.append(MessageTableEntry(message_id=message_id, flags=flags, text=text))
            cursor += length
    return entries


def _sanitize_symbol_suffix(name: str, used: set[str]) -> str:
    suffix = re.sub(r"[^0-9A-Za-z_]", "_", name)
...
def _section_for_rva(sections: list[PeSection], rva: int) -> PeSection | None:
...
def _read_c_string_at_va(data: bytes, sections: list[PeSection], image_base: int, va: int) -> str:
    offset = rva_to_offset(va - image_base, sections)
    if offset is None:
        raise ResourceExtractError(f"lookup-table string VA cannot be mapped: {hex32(va)}")
    end = data.index(b"\0", offset)
    return data[offset:end].decode("ascii", errors="replace")


def parse_zloc_lookup_rows(
    reference: Path,
    *,
    table_start_va: int = 0x10006030,
    table_end_va: int = 0x10007798,
) -> list[MessageLookupRow]:
    data = reference.read_bytes()
    headers = parse_pe_headers(data, source=str(reference))
    if table_end_va < table_start_va or (table_end_va - table_start_va) % 8 != 0:
        raise ResourceExtractError("lookup-table VA range must be an ascending 8-byte row range")
    start_rva = table_start_va - headers.image_base
    table_section = _section_for_rva(list(headers.sections), start_rva)
    if table_section is None:
        raise ResourceExtractError(f"lookup-table start VA cannot be mapped: {hex32(table_start_va)}")

    rows: list[MessageLookupRow] = []
    for row_va in range(table_start_va, table_end_va, 8):
        row_offset = rva_to_offset(row_va - headers.image_base, headers.sections)
        if row_offset is None:
            raise ResourceExtractError(f"lookup-table row VA cannot be mapped: {hex32(row_va)}")
        name_va, message_id = struct.unpack_from("<II", data, row_offset)
        rows.append(
            MessageLookupRow(
                name=_read_c_string_at_va(data, list(headers.sections), headers.image_base, name_va),
...
def message_table_resource(entries: list[ResourceEntry], data: bytes) -> tuple[ResourceEntry, bytes]:
    message_entries = [entry for entry in entries if entry.type_id == 11]
    if len(message_entries) != 1:
        raise ResourceExtractError(f"expected exactly one message-table resource, found {len(message_entries)}")
    entry = message_entries[0]
    return entry, data[entry.file_offset : entry.file_offset + entry.size]


def make_messages_mc(reference: Path, lookup_rows: list[MessageLookupRow], messages: list[MessageTableEntry]) -> str:
    lookup_by_id = {row.message_id: row.name for row in lookup_rows}
    message_by_id = {entry.message_id: entry for entry in messages}
    if len(lookup_by_id) != len(lookup_rows):
        raise ResourceExtractError("lookup table contains duplicate message ids")
    if len(message_by_id) != len(messages):
        raise ResourceExtractError("message table contains duplicate message ids")
    if set(lookup_by_id) != set(message_by_id):
        missing_lookup = sorted(set(message_by_id) - set(lookup_by_id))
        missing_message = sorted(set(lookup_by_id) - set(message_by_id))
        raise ResourceExtractError(
            "lookup/message ID mismatch: "
            f"resource-only={','.join(hex32(item) for item in missing_lookup[:8]) or 'none'} "
            f"lookup-only={','.join(hex32(item) for item in missing_message[:8]) or 'none'}"
        )

    lines = [
        f"; Generated from {reference.as_posix()} by python tools/recoil.py build resource --messages-mc.",
        "; Keep symbolic names, IDs, and text aligned with the retail messages.dll message table.",
        "",
    ...
    for message_id in sorted(message_by_id):
        ...
        lines.extend(
            [
                f"MessageId=0x{message_id:x}",
                f"SymbolicName={lookup_by_id[message_id]}",
                "Language=English",
            ]
        )
...
def make_messages_lookup_inc(reference: Path, lookup_rows: list[MessageLookupRow]) -> str:
    used: set[str] = set()
    rows: list[tuple[str, MessageLookupRow]] = [
        (_sanitize_symbol_suffix(row.name, used), row) for row in lookup_rows
    ]
    lines = [
        f"/* Generated from {reference.as_posix()} by python tools/recoil.py build resource --messages-lookup.",
        " * Keep the row order and writable name storage aligned with the retail messages.dll lookup table. */",
        "",
        "typedef struct MessagesLookupRow {",
        "    char *name;",
        "    unsigned int id;",
        "} MessagesLookupRow;",
        "",
    ...
    for suffix, row in rows:
        lines.append(f'static char g_MessageName_{suffix}[] = "{row.name}";')
    lines.extend(["", "static MessagesLookupRow g_MessagesLookupRows[] = {"])
    for suffix, row in rows:
        lines.append(f"    {{ g_MessageName_{suffix}, 0x{row.message_id:x} }},")
    lines.extend(
        [
            "};",
            "",
            f"#define MESSAGES_LOOKUP_ROW_COUNT {len(rows)}",
            "",
        ]
    )
    return "\n".join(lines)
...
def compare_res_payloads(reference: Path, candidate_res: Path) -> list[str]:
    ...
    for key in sorted(set(expected) | set(actual), key=lambda item: tuple(str(part) for part in item)):
        ...
        if expected_payload != actual_payload:
            shared = min(len(expected_payload), len(actual_payload))
            first_difference = next(
                (index for index in range(shared) if expected_payload[index] != actual_payload[index]),
                shared if len(expected_payload) != len(actual_payload) else None,
            )
            detail = ""
            if first_difference is not None:
                expected_byte = (
                    f"0x{expected_payload[first_difference]:02x}"
                    if first_difference < len(expected_payload)
                    else "<end>"
                )
                actual_byte = (
                    f"0x{actual_payload[first_difference]:02x}"
                    if first_difference < len(actual_payload)
                    else "<end>"
                )
                detail = (
                    f" first_difference=0x{first_difference:x} "
                    f"expected_byte={expected_byte} actual_byte={actual_byte}"
                )
            mismatches.append(
                f"{key}: payload differs "
                f"expected_size={len(expected_payload)} actual_size={len(actual_payload)}{detail}"
            )
    return mismatches
...
def manifest_for_entries(
    ...
    return {
        ...
        "payload_validation": {
            "method": "direct-live-bytes",
            "identity_fields": ["type_id", "name", "language"],
...
def make_rc_file(entries: list[ResourceEntry], img_dir: Path, repo_root: Path, data: bytes) -> str:
    lines = [
        ...
        "",
        "// Generated from support/Recoil.exe by python tools/recoil.py build resource.",
        "// Reconstructed from the original PE resource table into normal VC5SP3 .rc source.",
        "",
        "LANGUAGE 9, 1",
        "",
    ]
...
def extract_resources(
    ...
    messages_mc_path: Path | None = None,
    messages_lookup_path: Path | None = None,
) -> list[ResourceEntry]:
    repo_root = Path.cwd()
    ...
    if messages_mc_path is not None or messages_lookup_path is not None:
        _message_entry, message_payload = message_table_resource(written_entries, data)
        lookup_rows = parse_zloc_lookup_rows(reference)
        message_entries = parse_message_table(message_payload)
        if messages_mc_path is not None:
            write_text_if_changed(messages_mc_path, make_messages_mc(reference, lookup_rows, message_entries))
        if messages_lookup_path is not None:
            write_text_if_changed(messages_lookup_path, make_messages_lookup_inc(reference, lookup_rows))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--messages-mc", help="Write a source .mc file from a messages.dll message table.")
    parser.add_argument(
        "--messages-lookup",
        help="Write a C include with the messages.dll ZLocGetID symbol-name lookup rows.",
    )
    ...
    try:
        ...
        entries = extract_resources(
            ...
            Path(args.messages_mc) if args.messages_mc else None,
            Path(args.messages_lookup) if args.messages_lookup else None,
        )
