...
def compiler_root() -> Path:
    return Path(os.environ.get("RECOIL_COMPILER_ROOT", REPO_ROOT.parent / "Compiler"))


def local_vcvarsall_paths() -> list[Path]:
    root = compiler_root()
    return [
        root / "VS2022BuildTools" / "VC" / "Auxiliary" / "Build" / "vcvarsall.bat",
        root / "MSVC2022" / "VC" / "Auxiliary" / "Build" / "vcvarsall.bat",
        root / "VS2022" / "VC" / "Auxiliary" / "Build" / "vcvarsall.bat",
    ]
...
def candidate_vcvarsall_paths() -> list[Path]:
    ...
    candidates.extend(local_vcvarsall_paths())
